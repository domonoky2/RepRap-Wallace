...
end_heights = [40,55]; // Nema14, Nema17
...
!for(b = [0:1]) translate([b*60,0,0])  base_end()
...
end_height = end_heights[motor_type];
...
module leadscrew_coupler() difference() {
	...
	translate([0, 0, 3]) rotate([-90, 0, 90]) {
		...
		translate([-m3_nut_size / 2, -3, 0]) cube([m3_nut_size, 10, 5.7]);
	}
